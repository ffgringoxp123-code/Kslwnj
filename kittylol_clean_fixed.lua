    ...
    if scriptSource then
        ...
        queue_on_teleport([[
            ...
            local function isRelevantCaller()
                for i = 3, 6 do
                    local src = debug.info(i, "s")
                    if src and (src:find("RemoveLoadingScreen") or src:find("Loading") or src:find("Anti Abilities")) then
                        return true
                    end
                end
                if getcallingscript then
...
setModuleInteractable(true)
    ...
    local ColorpickerManager = {
        _color = settings.default or Color3.fromRGB(255, 20, 147),
        _h = 0, _s = 1, _v = 1
    }
...
local detectionstab = main:create_tab('Anti Abilities', 'rbxassetid://117218617893684')
...
local av = main:create_tab('Player', 'rbxassetid://117218617893684')
        ...
        if System.__properties.__is_mobile then
            if value then
                if not System.__properties.__mobile_guis.triggerbot then
                    local triggerbot_mobile = create_mobile_button('Trigger', 0.7, Color3.fromRGB(255, 255, 255))
...
local function create_curve_selector_mobile()
    ...
    toggle_btn.TextColor3 = Color3.fromRGB(0, 0, 0)
...
detectionstab:create_module({
    title = 'Death Slash Anti Abilities',
    flag = 'Death_Slash',
    description = '',
    section = 'right',
    callback = function(value)
...
detectionstab:create_module({
    title = 'Time Hole Anti Abilities',
    flag = 'Time_Hole',
    description = '',
    section = 'left',
    callback = function(value)
...
local slashes_module = detectionstab:create_module({
    title = 'Slashes Of Fury Anti Abilities',
...
-- Versão reforçada do Player Changer (mantive o modelo do script e assinaturas)
...
local ball_velocity = {
    __config = {
        ...
        colors = {
            ...
            accent_green = Color3.fromRGB(255, 255, 255),
            accent_orange = Color3.fromRGB(255, 255, 255),
...
local function unloadScript()
    ...
    local old_ailon = coreGui:FindFirstChild('ailon')
    if old_ailon then old_ailon:Destroy() end
